...
vax_cont_list[[13]]%>%
  filter(base_cont==0)%>%
  select(cont_chng,vax_round)

lmer(cont_chng ~ vax_cat2+StringencyIndex+age_cat1+hh_cat+(1|p_id), data = vax_cont_list[[13]])

df <- vax_cont_list[[13]]
df<- df%>%
      mutate(variant_worry_cat =factor(variant_worry_cat,
                                       levels = c("Decreased greatly","Decreased slightly","No change","Increased slightly","Increased greatly")))
m1 <- lmer(cont_chng ~ vax_cat2+StringencyIndex+variant_worry_cat+age_cat1+pol_cat+hh_cat+emp_cat+lca4_full+Series_Complete_Pop_Pct+(1|p_id), data = df)
mod_res(m1)
