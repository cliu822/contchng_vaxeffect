...
covariates <- c("Series_Complete_Pop_Pct",  "gender_i", "race_eth_i", 
               "pol_cat", "emp_cat", "income", "comorb_cat", 
                "lca4_full", "base_cont", "variant_worry_cat")
...
res_list_df<- res_list_df%>%
                mutate(formula = c(rep(covariates[1],each=3),rep(covariates[2:10], each=4)))%>%
                filter(vaxcat=="a2_full_new")

#######################################################
####Remove comorb cat, effect estimate at 2.1#########
#######################################################
...
#################################################
####Remove gender_i, effect estimate at 2.1######
#################################################
...
#################################################
####Remove income, effect estimate at 2.07######
#################################################
...
#################################################
####Remove race ethnicity, effect estimate at 2.04######
#################################################

# List of covariates to consider removing (excluding main exposure and random effects)
covariates <- c("Series_Complete_Pop_Pct", 
                "pol_cat", "emp_cat", 
                "lca4_full", "base_cont", "variant_worry_cat")
