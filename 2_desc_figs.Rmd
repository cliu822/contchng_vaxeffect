...
df_cont_cov<- df_cont_cov%>%
              mutate(income_cat =
                       case_when(income_cat=="0_24000"~"0-24,999",
                                 income_cat=="25000_74000"~"25,000-74,999",
                                 income_cat=="75000_149000"~"75,000-149,999",
                                 income_cat=="150000"~"150,000+",
                                 is.na(income_cat)~ NA_character_))

df_cont_cov <- df_cont_cov%>%
                mutate(race_eth_i =
                         case_when(race_eth_i=="1_Hisp"~"Hispanic",
                                   race_eth_i=="2_NH-White"~"Non-hispanic White",
                                   race_eth_i=="3_NH-Black"~"Non-hispanic Black",
                                   race_eth_i=="4_NH-Asian"~"Non-hispanic Asian",
                                   race_eth_i=="5_NH-Other"~"Non-hispanic Other"))

##Variable re-label
var_lab <- data.frame(var = c("age_cat1","pol_cat","emp_cat","gender_i","race_eth_i","income_cat","lca4_full","hh_cat","comorb_cat"),
            var_lab = c("Age group", "Political affiliation","Employment status","Gender","Race/ethnicity","Household income","Risk tolerance","Household size","Comorbidity"))%>%
            mutate(var_lab = factor(var_lab, 
                                    levels =  c("Age group","Gender", "Household size",
                                                "Race/ethnicity", "Political affiliation","Employment status",
                                                "Household income","Comorbidity","Risk tolerance")))

df_cont_cov_long <- df_cont_cov%>%
                        dplyr::select(round, p_id, val99_all, age_cat1:lca4_full,income_cat)%>%                      #Select round, all contacts 99th percentile, covariate 
                        pivot_longer(cols = age_cat1:income_cat, names_to="var",values_to="val")%>%
                         ...
                         mutate(val = ifelse(val %in% c("Unknown Pol", "Unknown Emp"), NA_character_,val),
                                val = factor(val,
                                                          levels = c("18-24","25-34","35-44","45-54","55-64","65+",
                                                                     ...
                                                                     "0-24,999","25,000-74,999","75,000-149,999","150,000+",
                                                                     "Yes","No",
...
##Set color panels
pal <- c(rev(brewer.pal("Oranges", n=9))[c(1,2,3,5,6,7)],
         ...
         brewer.pal("Blues",n=3),
         brewer.pal("Purples",n=9)[c(2,3,5,6,7)],
         brewer.pal("YlOrBr",n=9)[c(2,4)],
         brewer.pal("Greens", n=9)[c(2,4,6,8)])
...
for(i in 1:9){
  ...
  p[[i]]<-cont_cov_list[[i]]%>%
            ...
            theme_bw()+ylim(0,28) +
            theme(legend.position = c(0.28, 0.78), legend.title=element_blank(),
...
png("0_plot/1_contact_trajectories_covariate_v4.png",width=10,height=10, units="in",res=300)
 grid.arrange(
  arrangeGrob(grobs = p,ncol=3,nrow=3,
             bottom=textGrob("Round",gp=gpar(fontsize=16)),
             left=textGrob("Average number of contacts per person per day",rot = 90,gp=gpar(fontsize=16))))
dev.off()
```
...
png("0_plot/2_meancont_vaxdose.png",width=8,height=5, units="in",res=300)
p1b+ggtitle("Contact rates by vaccine doses received")
dev.off()
...
png("0_plot/1_changevaxcat_v2.png",width=8,height=3, units="in",res=300)
p1c+ggtitle(" Distribution of changes in vaccination status")
dev.off()
