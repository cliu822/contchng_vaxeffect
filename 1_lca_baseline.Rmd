...
```{r}
df_modclass_sum <- df_behav%>%dplyr::select(social_distant__r1, travel_essential__r1,travel_nonessential__r1, facemask2__r1,soapcat__r1, sanicat__r1,vaccine__r1,
                          lca3_full:lca5_mod3)%>%
...
##Table of survey responses and LCA category
```{r}
df_behav <- df_behav%>%
  mutate(soapcat2__r1 = case_when(
    is.na(soap__r1)~NA_character_,
  ...
  sanicat2__r1 = case_when(
    is.na(sanitize__r1)~NA_character_,
  ...
  mutate(soapcat2__r1 = factor(soapcat2__r1, levels = c("0-4 times","5-6 times","7-10 times", "11 or more times")),
         sanicat2__r1 = factor(sanicat2__r1, levels = c("0 times","1-3 times","4-5 times","6 or more times")))

df_lca4_cov <- df_behav%>%
  dplyr::select(lca4_full, socdist_cat__r1:transcat__r1,soapcat2__r1, sanicat2__r1)%>%
  pivot_longer(cols=socdist_cat__r1:sanicat2__r1, names_to="var",values_to="val")%>%
  group_by(lca4_full,var,val)%>%
  dplyr::summarise(n=n())

tab_lca4_cov <- df_lca4_cov%>%
...
write.csv(tab_lca4_cov, "0_tab/tab_lca4_cov.csv")
